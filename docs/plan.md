...
- Constitution version: v1.0 — [docs/constitution.md](constitution.md). Status: APPROVED.
- Approval evidence: both approved by Tóth Tibor, 2026-07-14 (see each
  document's Human spec gate / Constitution gate section).
...
- Scope/file ownership (per `constitution.md` → Non-negotiable boundaries):
  ...
  `docs/**`; a new gitignored SQLite data file (path decided in TASK-01/TTO-5).
...
- Constraints discovered: no database, no API routes, and no
  `ANTHROPIC_API_KEY`/`.env.local` exist yet; `better-sqlite3` is not yet a
  dependency — adding it is the one pre-agreed exception to the "no new
  dependency" rule (chosen over a JSON file or Neon Postgres — see Risks below).
...
- Plan assumptions that require validation:
  - Single-user, single-process local SQLite file has no concurrent-writer
    risk in this MVP (no auth, one demo user — per constitution).
...
|---|---|---|---|---|
| 1 | SQLite schema + data module: `conversations`, `messages` tables — `src/lib/db.ts` (Linear TTO-5) | AC-04 | `npm run test -- db` | Tóth Tibor |
...
- Schema/migration impact: first schema in the repo — `conversations` and
  `messages` tables (Linear TTO-5); no prior schema to break.
- Authorization/privacy impact: none in scope — no auth (constitution
  non-goal); only user-entered chat text is stored, locally, unencrypted, on a
  gitignored file.
...
- Chosen approach and why: SQLite via `better-sqlite3` — synchronous,
  zero external service, fits a single-user local MVP; chosen over
  alternatives during spec discussion.
- Alternative rejected and why: plain JSON file — rejected as less robust for
  a growing message history; Neon Postgres — rejected for this MVP, since
  hosting/DB integration is explicitly not-yet-wired per
...
- [x] Spec and constitution versions are approved. — v1.0 each, Tóth Tibor, 2026-07-14.
...
- [x] Reviewer roles are selected. — a separate, fresh-context Claude Code or
      ...
      the builder session).
- [x] Human approved the plan or recorded an owned exception. — Tóth Tibor, 2026-07-14.
